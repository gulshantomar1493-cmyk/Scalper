...
Scope note: the V1/V2/V3/V4 object streams this file used to hash were the
V1 analysis-engine payload (structure, liquidity, order blocks, gaps) and
the V1 signals/recommendations. Those layers were removed in the V4 cutover
(docs/V4/ARCHITECTURE.md §6), so what remains is the guarantee that still
matters: the CANDLE stream V4 reads from is byte-identical across replays.
V4's own determinism is covered by test_v4.py::test_deterministic.
"""
