#!/usr/bin/env python
"""Local 1m backfill (owner-operated, offline).

Fills missing canonical 1m candles in the LOCAL dev DB from Binance public
klines, so replay and the V4 strategy layer can run over long ranges locally
(prod already holds 9 years). Read-only against Binance; append-only inserts
(existing days are skipped by a day-level coverage check; duplicate rows are
skipped one-by-one).

Usage: python scripts/backfill_local.py --days 400
"""
