...
(function () {
  ...
  function init(c, saved) { chart = c; cfg = merge(saved); legendEl = document.getElementById("ind-legend"); ensure(); renderLegend(); }
  ...
  function renderMenu(panel, onChange) {
    ...
    /* A switch with no text needs a name and a state, or a screen reader
       announces nothing but "button". */
    const toggle = (get, set, name) => {
      const b = mk("button", "ind-tog"); b.type = "button";
      b.setAttribute("role", "switch");
      b.setAttribute("aria-label", name || "toggle");
      const paint = () => { const on = get(); b.classList.toggle("on", on);
    ...
    SLOTS.forEach(i => row("MA", [
      toggle(() => cfg.ema[i].on, v => cfg.ema[i].on = v, "moving average " + (i + 1)),
      num(() => cfg.ema[i].len, v => cfg.ema[i].len = v, true),
      color(() => cfg.ema[i].color, v => cfg.ema[i].color = v),
    ]));
    row("SMA", [toggle(() => cfg.sma.on, v => cfg.sma.on = v, "SMA"), num(() => cfg.sma.len, v => cfg.sma.len = v, true), color(() => cfg.sma.color, v => cfg.sma.color = v)]);
    row("RSI", [toggle(() => cfg.rsi.on, v => cfg.rsi.on = v, "RSI"), num(() => cfg.rsi.len, v => cfg.rsi.len = v, true),
    ...
    row("Volume", [toggle(() => cfg.volume.on, v => cfg.volume.on = v, "volume")]);
  }
